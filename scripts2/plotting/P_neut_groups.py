...
def P_neut_groups(PK_data, PNeut_location, result_location, Lin_groups, col_groups, antigen):
    ...
    def Display_Envelope(t, Y, Z, is_log, labels, figsize = (7, 7), xysize = (15,15), labsize = 20, save_to = "test", xval = "x", yval = "f(x)", 
        ...
        if labels != [""]:
           ax.legend(loc = (0.25, -0.3) ,fontsize = labsize, ncols = 2)#np.ceil(len(labels)/4))
        ...
        if save_to is not None:
            ...
            fig.savefig(save_to+".png", bbox_inches = "tight")
    ...
    Lin_list = []
    col_list = []
    for i in range(len(Lin_groups)):
        Lin_i_list = Lin_groups[i]
    ...
    fig, ax = Display_Envelope(t, np.array(Min_list), np.array(Max_list), 
                              is_log, 
                              save_to = Res_dir+"/P_Neut_groups_%s"%antigen,
                              xval = xval, yval = yval,
                              linewidth = 4,
                              color = col_list,
                              alpha = 0.3,

...
def Cross_maps(filename, special_lin=None, cross_special_lin = None, result_location = "", plot_major = None, use_special_lin = "not_add"):
    
    try:
        plot_major = str(plot_major)
    except:
        plot_major = None
    ...
    if plot_major not in ("True"):
        ...
        try:
            Lin_name = special_lin
            if Lin_name not in Top_Pseudo:
                cross_lin = open(cross_special_lin, "rb")
        ...
        for k in range(len(choosen_Ab)):
            ...
            pdf = PdfPages(result_location+"/Cross_React_AB_%s.pdf"%ab)
            pdf.savefig(fig_fr, bbox_inches = "tight")
            fig_fr.savefig(result_location+"/Cross_React_AB_%s.svg"%ab, bbox_inches = "tight")
            pdf.close()
        ...
        for k in range(len(choosen_Ab)):
            ...
            dLab = "Cross-Resistance to %s (log 10)"%ab
            ...
            pdf = PdfPages(result_location+"/major_Cross_React_AB_%s.pdf"%ab)
            pdf.savefig(fig_fr, bbox_inches = "tight")
            fig_fr.savefig(result_location+"/major_Cross_React_AB_%s.svg"%ab, bbox_inches = "tight")
            fig_fr.savefig(result_location+"/major_Cross_React_AB_%s.png"%ab, bbox_inches = "tight")
            pdf.close()
            
        if os.path.exists(str(filename)[:-4]+"_weighted.pck"):
            ### plot Cross reactivity between major variant groups
            file0 = open(str(filename)[:-4]+"_weighted.pck", "rb") 
            Cross_show=pickle.load(file0)
            file0.close()
            Top_Pseudo_var = list(Cross_show["variant_list"])
            Cross_show.pop("variant_list")
            ...
            for k in range(len(choosen_Ab)):
                ...
                pdf = PdfPages(result_location+"/major_Cross_React_AB_%s_weighted.pdf"%ab)
                pdf.savefig(fig_fr, bbox_inches = "tight")
                fig_fr.savefig(result_location+"/major_Cross_React_AB_%s_weighted.svg"%ab, bbox_inches = "tight")
                fig_fr.savefig(result_location+"/major_Cross_React_AB_%s_weighted.png"%ab, bbox_inches = "tight")
                pdf.close()

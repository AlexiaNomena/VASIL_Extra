...
file1 = open(sys.argv[1], "rb") # premade simulations
...
file1 = open(sys.argv[2], "rb") # load cross reactivity if parameter is not a directory
...
Load_Delta = pd.read_excel(sys.argv[4], engine='openpyxl')
...
k = 6
while k<6+num_groups:
    Lin_list.append(str(sys.argv[k]))
    k+=1

num_antigen = int(sys.argv[k])
antigen_list = []
for i in range(num_antigen):
    antigen_list.append(sys.argv[k+1+i])
...
def pneut_util(Lin_name, variants_in_cross, antigen_list,
               Cross_react_dic = None, w_save=len(sys.argv)-1, var_name = None):
...
for lin in Lin_list:
    pneut_util(lin, variants_in_cross, antigen_list, Cross_react_dic, w_save=len(sys.argv)-1)
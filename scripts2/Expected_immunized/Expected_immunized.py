...
def Expected_Immunized(country, case_data, column_case, cross_delta, cross_file, 
                       spikegroup_freqs, spikegroup_list, dms_data,
                       VE_data, total_population, date_ranges, lineages_list,
                       result_dir, num_antigen = 0, antigen_list = ["none"], save_pneut = False, datadir = "data/"):
    
    """
    #### Arguments readapted from scripts/Expected_immunized/Expected_immunized.py
    sys.argv[1] = datadir+country+case_data = case_ascertainement data             
    sys.argv[2] = cross_delta = cross_neutralization to delta VE validataion (obtained in pipeline or with specific formats)
    sys.argv[3] = cross_file = Cross_reactivity.pck or a directory locations of Cross...pck for variant group simulation (obtained in pipeline or with specific formats)
    sys.argv[4] = spikegroup_freqs = Spikegroups frequencies (obtained in pipeline or with specific formats)
    sys.argv[5] = spikegroup_list = Spikegroups list as pck file (obtained in pipeline or with specific formats)
    sys.argv[6] = dms_data = dms_per_ab_per_site.csv (obtained in pipeline or with specific formats)
    sys.argv[7] = VE_data = vaccine efficacy data for IC50 parameter estimation
    sys.argv[8] = total population size
    sys.argv[9] = date_ranges[0] = date_start simulation
    sys.argv[10] = date_ranges[1] = date_end simulation
    sys.argv[11] = result_dir = result directory
    sys.argv[12] = save_pneut = bool to save P_neut to antigen or not
    sys.argv[13] of sys.argv[13:13+num_groups] = lineages_list = one lineage (num_groups = 1),"ALL" for all spikegroups (num_groups = 1), list of lineages compare_groups params (num_groups != 0)
    sys.argv[13+num_groups] = number of antigens in antigen_list
    sys.argv[13+num_groups:14+num_groups+num_antigen] = antigen_list
    sys.argv[len(sys.argv) - 1] = column_case = str of column name used as case ascertainment
    """
    
    """Load Infection Data"""
    Population_Data = pd.read_csv(datadir+"/%s/%s"%(country,case_data))
    ...
    try:
        file1 = open(cross_file, "rb") # load cross reactivity if parameter is not a directory
    ...
    PK_df.to_csv(datadir+"%s/"%country+"results/PK_for_all_Epitopes.csv")
    ...
    def ei_util(Lin_name, variants_in_cross, antigen_list,
                Cross_react_dic = None, infection_data = infection_data_corrected, save_pneut=None, w_save=None, 
        ...
        if len(antigen_list) != 0:
            if save_pneut in ("TRUE", "True"):
                ...
                VE_df.to_csv(result_dir+"/P_neut_"+Lin_name+".csv")
            else:
                pass
        ...
        try:
            ...
            if w_save is not None:
                ...
                if var_name is not None:
                    EI_df.to_csv(result_dir+"/Immunized_SpikeGroup_%s_all_PK.csv"%var_name)
                
                    if save_suscept:
                        Susc_df.to_csv(result_dir+"/Susceptible_SpikeGroup_%s_all_PK.csv"%var_name)
                else:
                    EI_df.to_csv(result_dir+"/Immunized_SpikeGroup_%s_all_PK.csv"%variant_to_sim[0])
                    if save_suscept:
                        Susc_df = pd.DataFrame(Susc)
                        Susc_df.to_csv(result_dir+"/Susceptible_SpikeGroup_%s_all_PK.csv"%variant_to_sim[0])
                return "Done"
            else:
    ...
    w_save = 11 # index of resdir (here it's just a placeholder since we are already providing the directory as a parameter)
    save_pneut = str(save_pneut)
    """Load Lineage to assess """
    Lin_name = lineages_list
    """Update name if necessary -- this is the same update as in Compute_FR"""
    
    ### Load spikegroups membership file
    file = open(datadir+"/%s"%country+"Spikegroups_membership.pck", "rb")
    Pseudogroup_dic = pickle.load(file)
    file.close()
    if Lin_name not in ("ALL", "ALL_vs_Vacc_ver1", "ALL_vs_Vacc_ver2"):
        if not run_group or str(cross_file[:31]) == datadir+"%s/"%country+"results/Cross_react_dic_groups_" or str(cross_file[:22]) == "vaccination/Cross_Vacc" or str(cross_file[:20]) == "outbreak/Cross_files":
            num_antigen = num_antigen
            k=15
            antigen_list = antigen_list
            #while k<15+num_antigen:
            #    antigen = str(antigen_list[k])
            #    antigen_list.append(antigen)
            #    k+=1
                
            Grouped = False
            if Lin_name not in list(Pseudogroup_dic.keys()):
                if str(cross_file[:31]) == datadir+"%s/"%country+"results/Cross_react_dic_groups_":
                    file1 = open(datadir+"%s/"%country+cross_file+"results/Cross_%s.pck"%Lin_name, "rb") # load cross reactivity of a group with mutation profile extracted from a small covsonar data file
                    Cross_react_dic = pickle.load(file1)
                    variants_in_cross = Cross_react_dic["variant_list"]
                    Cross_react_dic.pop("variant_list")
                    file1.close()
                    ...
                    Grouped = True
                elif str(cross_file[:22]) == "vaccination/Cross_Vacc": ## hard-coded for vaccination pseudo variants
                    ...
                    for i in range(len(lineages)):
                        var = lineages[i]
                        file1 = open(datadir+"%s/"%country+"results/"+cross_file+"/Cross_%s.pck"%var, "rb")
                        Cross_react_dic = pickle.load(file1)
                        variants_in_cross = Cross_react_dic["variant_list"]
                        Cross_react_dic.pop("variant_list")
                        file1.close()
                ...
                elif str(cross_file[:20]) == "outbreak/Cross_files": ## hard-coded for vaccination pseudo variants
                ...
                try:
                    file1 = open(datadir+"%s/"%country+"results/Cross_react_dic_spikegroups_ALL.pck", "rb") # Check that this is the file you want to load
                    Cross_react_dic = pickle.load(file1)
                    ...
                    try:
                        file1 = open(datadir+"%s/"%country+"results/Cross_react_dic_spikegroups_present.pck", "rb") # Check that this is the file you want to load
                        Cross_react_dic = pickle.load(file1)
                        variants_in_cross = list(Cross_react_dic["variant_list"])
                        ### insert lin_sim in the position of it's Pseudogroup
                        Cross_react_dic["variant_list"][variants_in_cross.index(Pseudogroup_dic[Lin_name])] = Lin_name
            ...
            if not Grouped:
                ...
                sim_df.to_csv(result_dir+"/simulation_status_%s.csv"%Lin_name)
            else:
                sim_df = pd.DataFrame({"Lineage": lineages, "Simulation status":status_var})
                sim_df.to_csv(result_dir+"/simulation_status.csv")
        else:
            Lin_list = []
            cross_list = []
            k = 0
            run_k = True
            not_pres = []
            while k<len(lineages_list) and run_k:
                lin_sim = str(lineages_list[k])                
                try:
                ...
                if run_k:
                    try:
                        file1 = open(cross_file+"/Cross_%s.pck"%lin_sim, "rb") # Check that this is the file you want to load
                        ...
                        try:
                            try:
                                file1 = open(datadir+"%s/"%country+"results/Cross_react_dic_spikegroups_%s.pck"%lin_sim, "rb") # Check that this is the file you want to load
            ...
            for i in range(len(Lin_list)):
                ...
                # Save file as a placeholder for exectuted codes, required for snakemake
                if i != 0:
                    sim_df = pd.read_csv(result_dir+"/simulation_status_group.csv")
                ...
                sim_df.to_csv(result_dir+"/simulation_status_group.csv")    
        ...
        if Lin_name not in ("ALL_vs_Vacc_ver1", ):
            ...
            if len(SpikeGroups_list_index)!=len(SpikeGroups_list):
                 ...
                 if os.path.exists(datadir+"%s/"%country+"results/Cross_react_dic_spikegroups_present.pck"):
                    file1 = open(datadir+"%s/"%country+"results/Cross_react_dic_spikegroups_present.pck", "rb") 
                    Cross_react_dic = pickle.load(file1)
                    variants_in_cross = Cross_react_dic["variant_list"]
                    Cross_react_dic.pop("variant_list")
                    file1.close()
        ...
        for i in range(len(SpikeGroups_list)):
            if SpikeGroups_list[i] in variants_in_cross or SpikeGroups_list[i] in list(Pseudogroup_dic.keys()):
                if Lin_name not in ("ALL_vs_Vacc_ver2", ):
                    ...
                    EI_df.to_csv(result_dir+"/Immunized_SpikeGroup_%s_all_PK.csv"%SpikeGroups_list[i]) 
                    Susc_df.to_csv(result_dir+"/Susceptible_SpikeGroup_%s_all_PK.csv"%SpikeGroups_list[i])
        ...
        if Lin_name not in ("ALL_vs_Vacc_ver2", ):
            ...
            sim_df.to_csv(result_dir+"/simulation_status_ALL.csv")
        else:

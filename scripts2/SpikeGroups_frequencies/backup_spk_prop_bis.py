...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 17 00:07:06 2023
...
Lineages_list_orig = list(Lineages_list)
variant_proportion_orig0 = variant_proportion_orig.copy()
...
for x in range(len(unique_group)):
    if "/" not in unique_group[x]:
        ...
        for var_x in splited_var:
            ...
            elif var_x in Lineages_list_orig:
                in_old_orig.append(list(Lineages_list_orig).index(var_x))
            
        if len(where_x) != 0:
            if len(in_old_orig)!=0:
                variant_proportion.append(np.sum(variant_proportion_orig[np.array(where_x), :], axis = 0) +  np.sum(variant_proportion_orig0[np.array(in_old_orig), :], axis = 0))
            else:
